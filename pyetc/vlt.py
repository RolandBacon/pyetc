...
class VLT(ETC):
    ...
    def __init__(self, log=logging.INFO):
        ...
        # IFS BlueMUSE
        self.ifs['blue'] = dict(spaxel_size = 0.20, # spaxel size in arcsec
        ...
        filename = 'NewBlueMUSE_noatm.txt'
        ...
        # IFS MUSE
        self.ifs['red'] = dict(spaxel_size = 0.20, # spaxel size in arcsec
                               dlbda = 1.25, # Angstroem/pixel
                               lbda1 = 4650, # starting wavelength in Angstroem
        ...
        filename = 'WFM_NONAO_N_noatm.txt'
        ifs_trans=np.loadtxt(os.path.join(CURDIR,filename))
        self.ifs['red']['instrans'] = Spectrum(data=np.interp(self.ifs['red']['atmtrans'].wave.coord(),ifs_trans[:,0]*10.0,ifs_trans[:,1]), 
